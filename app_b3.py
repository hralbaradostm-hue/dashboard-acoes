        ...
        with st.spinner(f"Buscando {rx_ticker} na Bolsai..."):
            ...
            try:
                ...
                if res.status_code == 200:
                    ...
                    # O Yahoo Finance exige que ações brasileiras tenham ".SA" no final (ex: PETR4.SA)
                    ticker_yf = f"{rx_ticker}.SA"
                    
                    with st.spinner("Buscando histórico na bolsa..."):
                        try:
                            # Baixa os dados do último 1 ano
                            historico = yf.download(ticker_yf, period="1y", progress=False)
                            
                            if not historico.empty:
                                # Cria o gráfico de linha interativo
                                fig = px.line(
                                    historico, 
                                    x=historico.index, 
                                    y='Close', 
                                ...
                                st.warning("Não foi possível carregar o histórico desta ação no Yahoo Finance.")
